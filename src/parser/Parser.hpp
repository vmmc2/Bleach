 ...
 * previous stage of the interpreting pipeline and generating (if the received list of tokens is valid) an AST
 * (Abstract Syntax Tree) that reflects the structure of the program written in the Bleach language. If the
 * generated sequence of tokens is not a valid Bleach program, one or more syntax errors are going to be reported
 * to the user and then the AST will not be generated, leading to the return of a nullptr instead of the syntax
 * tree.
 * 
 * @note As mentioned above, the parser expects the received sequence of tokens to be valid. If that's not the
 * case, then syntax errors are going to be reported to the user and a nullptr will be returned as the result of
...
class Parser{
  private:
     ...
     * 
     * @return The token that has just been consumed by the parser.
    **/
    Token previous(){
     ...
     *
     * This method is responsible for returning the first token that the parser has yet to consume. In other
     * words, the token that is being currently pointed by the 'current' attribute of the 'Parser' class.
     * 
    ...
    **/
    ...
    /**
     * @brief Checks whether or not the parser has reached the end of its list of tokens.
     *
     * This method is responsible for checking if the parser has reached the end of its list of tokens.
     * 
     * @return A boolean that signals whether or not the parser has reached the end of the list of tokens.
    **/
    bool isAtEnd(){
     ...
     * attention to the fact that this method never consumes a token. It only looks at it (remember the concept
     * of a lookahead).
     * 
     * @param type: The type of token which we will use to check whether or not the next token has such value as
     * its type of token.
     * 
    ...
    **/
    ...
    /**
     * @brief Reports a syntax error (parsing error) and returns an instace of the "ParseError" struct back to
     * its caller.
     *
     * This method is responsible for reporting a syntax error (parsing error) to the user through the console
     * (terminal) and also returning an instance of the "ParseError" struct back to its caller. 
     * 
     * @param token: A token that is the cause of the syntax error (parsing error) discovered by the parser.
     * @param errorMessage: A string that has the contents of the error message that must be shown to the user
     * in the console (terminal) if the token that's about to be consumed doesn't have the expected type.
     * 
     * @return An instance of the "ParseError" struct with an empty message.
    **/
     ...
     *
     * This method is responsible for discarding tokens that haven't been consumed yet by the parser from the
     * sequence of tokens until it hits what is most likely a statement boundary or the end of the tokens
     * sequence.
     * 
     * @return Nothing (void).
    **/
    ...
    /**
     * @brief Checks to see if the next token to be consumed is of the provided type. If so, it consumes the
     * token and everything is fine. Otherwise, a syntax error (parsing error) is reported and thrown.
     *
     * This method is responsible for checking whether the next token to be consumed by the parser is of the
     * provided type. If that's the case, then the parser consumes it and goes on with the parsing process.
     ...
     * is of the same type as the provided type. Otherwise, a syntax error (parsing error) is thrown.
     * 
    ...
    **/
     ...
     *
     * This method is responsible for checking whether the parser has reached the end of its token sequence.
    ...
    **/
     ...
     *
     * This method receives a list/sequence of types of tokens and checks whether the next token (the token 
     * that has not been consumed by the parser yet) has a type equal to one of the provided values in the
     * list of types of tokens. If it has, then the token is consumed and returns true. Otherwise, the token is not
     * consumed and the method returns false.
     * 
     * @param type: A sequence of comma separated types of tokens.
     * 
     * @return A boolean that signals whether the next token (that hasn't been consumed by the parser yet) has
     * a type that is equal to one of the types provided as arguments to this method.
    **/
    template<class... T>
    bool match(T... type){
    ...
    **/
    ...
    /**
     * @brief Represents the "breakStmt" rule inside the CFG of the Bleach language.
     *
     * This method is responsible for representing the "breakStmt" rule from the Context-Free Grammar of the
     * Bleach language. To understand better what the method is doing, take a look at Bleach's CFG.
     * 
     * @return An instance of the Stmt struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    **/
    ...
    /**
     * @brief Represents the "classDeclStmt" rule inside the CFG of the Bleach language.
     *
     * This method is responsible for representing the "classDeclStmt" rule from the Context-Free Grammar of the
     * Bleach language. To understand better what the method is doing, take a look at Bleach's CFG.
     * 
     * @return An instance of the Stmt struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    **/
    std::shared_ptr<Stmt> classDeclStatement(){
      ...
      std::shared_ptr<Variable> superclass = nullptr; // Assume that the user-defined class has no superclass.
      if(match(TokenType::INHERITS)){ // This keyword tells us that, if everything is correct, this user-defined class has indeed a superclass.
      ...
      while(!check(TokenType::RIGHT_BRACE) && !isAtEnd()){
        methods.push_back(funcDeclStatement("method")); // Store the methods that were declared inside the user-defined class.
      }
    ...
    /**
     * @brief Represents the "continueStmt" rule inside the CFG of the Bleach language.
     *
     * This method is responsible for representing the "continueStmt" rule from the Context-Free Grammar of the
     * Bleach language. To understand better what the method is doing, take a look at Bleach's CFG.
     * 
     * @return An instance of the Stmt struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    ...
    **/
    ...
    /**
     * @brief Works as an auxiliary method to the "call" method shown below this one.
     *
     * This method is works as a helper method for the "call" method shown below. This method receives the 
     * callee, which is an expression that is expected to evaluate into a callable entity during runtime, and
     * tries to parse the list of arguments present in the call expression. If the whole parsing process goes
     * fine, the method returns an instance of Call struct.
     * 
     * @param callee: An expression that supposedly represents, at runtime, a callable entity (a class, a function,
     * a lambda function, a method or a native function).
     * 
     * @return An instance of the Expr struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    **/
    ...
    /**
     * @brief Represents the "call" rule inside the CFG of the Bleach language.
     *
     * This method is responsible for representing the "call" rule from the Context-Free Grammar of the
     * Bleach language. To understand better what the method is doing, take a look at Bleach's CFG.
     * 
     * @return An instance of the Expr struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    ...
    **/
    ...
    /**
     * @brief Represents the "lambdaFunctionExpr" rule inside the CFG of the Bleach language.
     *
     * This method is responsible for representing the "lambdaFunctionExpr" rule from the Context-Free Grammar
     * of the Bleach language. To understand better what the method is doing, take a look at Bleach's CFG.
     * 
     * @return An instance of the Expr struct representing a node of Abstract Syntax Trees (AST) from the Bleach
     * language for this rule.
    **/
